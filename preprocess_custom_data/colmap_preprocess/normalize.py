...
def refine_visual_hull(masks, Ps, scale, center):
    ...
    GRID_SIZE=100
    MINIMAL_VIEWS=10 # Set according to specific dataset
...
def get_normalization(source_dir):
    ...
    cameras_filename = "cameras_xinzhu"
    ...
    normalization, _ = get_normalization_function(Ps, mask_points_all, number_of_normalization_points, number_of_cameras,masks_all)
...
def xinzhu_get_normalization(source_dir, cameras_data):
    number_of_normalization_points = 100

    masks_dir = '{0}/mask'.format(source_dir)
    mask_points_all, masks_all = get_all_mask_points(masks_dir)
    number_of_cameras = len(masks_all)
    Ps = get_Ps(cameras_data, number_of_cameras)

    normalization, _ = get_normalization_function(Ps, mask_points_all, number_of_normalization_points, number_of_cameras,masks_all)
    print(normalization)
    
    for i in range(number_of_cameras):
        cameras_data['scale_mat_%d' % i] = normalization
    
    return cameras_data

if __name__ == "__main__":
    source_dir = sys.argv[1]
    get_normalization(source_dir)
    print('Done!')